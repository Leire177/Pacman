...
var GF = function(){
 ...
 // variables para contar frames/s, usadas por measureFPS
	...
	Pacman.prototype.move = function() {
		...
		var topeDer= w-this.radius;
		var topeIzq = this.radius;

			pacman.draw(this.posX,this.posY);
			if(pDer){
				if(this.posX<topeDer){
     ...
     Pacman.prototype.draw = function(x, y) {
       ...
       y=y+this.radius;
	    ...
	    // Tu código aquí  
	    ctx.beginPath();
   		ctx.arc(x, y, 15, pacman.angle1*Math.PI, pacman.angle2*Math.PI,false);
    	ctx.lineTo(x, y);
    	ctx.closePath();
